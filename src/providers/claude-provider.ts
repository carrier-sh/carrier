...
`;
  ...
  protected generateAgentContext(config: TaskConfig): void {
    ...
    // Save initial context to filesystem
    fs.writeFileSync(contextPath, JSON.stringify(context, null, 2));
    console.log(`📝 Context initialized: runs/${this.currentRunId}/context.json`);

    // Send context to API for database storage (async, non-blocking)
    this.sendContextToAPI(config.deployedId, context).catch(err => {
      // Already handled in sendContextToAPI, just prevent unhandled rejection
    });
  }
  ...
  protected updateAgentContext(deployedId: string, taskId: string, update: any): void {
    ...
    try {
      ...
      // Send updated context to API for database storage (async, non-blocking)
      this.sendContextToAPI(deployedId, context).catch(err => {
        // Already handled in sendContextToAPI, just prevent unhandled rejection
      });
    } catch (e) {
...
`;
  ...
  /**
   * Send context data to API for database storage
   * Fails silently if API is unavailable
   * Uses authentication if available in auth.json
   */
  private async sendContextToAPI(deploymentId: string, contextData: any): Promise<void> {
    const apiUrl = process.env.CARRIER_API_URL || 'http://localhost:3000';
    const endpoint = `${apiUrl}/api/deployments/${deploymentId}/context`;
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`⚠️  Failed to send context to API: ${response.status} ${response.statusText}`);
      } else {
        const result = await response.json();
        const authStatus = authToken ? ' (authenticated)' : ' (unauthenticated)';
        console.log(`✓ Context sent to API${authStatus}: ${result.message || 'Success'}`);
      }
    } catch (error: any) {
      // Fail silently - API might not be available in local dev
      console.debug(`Context API unavailable: ${error.message}`);
    }
  }
